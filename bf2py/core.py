import sys
from .exceptions import BF2PySyntaxError, BF2PyRuntimeError
...
class BF2Py:
    ...
    def __init__(self, memory_size=30000):
        """
        Initialize the BF2Py interpreter.
    ...
    def _validate_code(self, code):
        ...
        Raises:
            BF2PySyntaxError: If brackets are unbalanced
        """
        stack = []
        for i, char in enumerate(code):
            if char == '[':
                ...
                if not stack:
                    raise BF2PySyntaxError(f"Unmatched ']' at position {i}")
                stack.pop()
        
        if stack:
            raise BF2PySyntaxError(f"Unmatched '[' at position {stack[0]}")
    ...
    def compile(self, brainfuck_code):
        ...
        Raises:
            BF2PySyntaxError: If the Brainfuck code has syntax errors
        """
    ...
    def execute(self, brainfuck_code, input_data=""):
        ...
        Raises:
            BF2PySyntaxError: If the Brainfuck code has syntax errors
            BF2PyRuntimeError: If there's a runtime error during execution
        """
        try:
        ...
        except BF2PySyntaxError:
            raise
        except Exception as e:
            raise BF2PyRuntimeError(f"Runtime error: {str(e)}")
